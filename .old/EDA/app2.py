...
columns = ["Date", "Type", "Price (US$)", "Notes"]
...
# Optional: Color peaks and troughs differently
for i, row in enumerate(table_data, start=1):
    color = '#ffcccc' if row[1] == 'Peak' else '#cce5ff'
    for j in range(len(columns)):
        table[(i, j)].set_facecolor(color)

plt.title("Brent Oil Price Turning Points", fontsize=14, pad=20)
plt.tight_layout()
plt.show()
