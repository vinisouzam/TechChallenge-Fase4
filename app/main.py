...
st.set_page_config(page_title="Previsões do Brent", layout="wide")
...
if current_page == "forecast":
    ...
    st.markdown(
        '''<h1> Avaliação do modelo que está sendo selecionado. </h1>'''
        ,unsafe_allow_html=True)
    
    df_modelvalidation = pd.read_csv(r'./models/model_evaluation.csv')
    
    df_modelvalidation.sort_values(by=['MAE','RMSE'],ascending=[True,True],
                                   inplace=True)
    
    # st.dataframe(df_modelvalidation, hide_index=True)
    
    st.markdown(
        '''<p> Como podemos observar, 
        o melhor modelo com base no MAE foi {}
        </p>'''.format(df_modelvalidation.iloc[0,0])
        ,unsafe_allow_html=True)
    
        # Converter para formato longo
    df_long_validation = df_modelvalidation.melt(
        id_vars='Model', value_vars=['MAE', 'RMSE'],
        var_name='Métrica', value_name='Erro')

    # Gráfico
    fig = px.bar(df_long_validation, x='Model', y='Erro', color='Métrica',
                barmode='group',title="Comparação de MAE e RMSE por Modelo", 
                text='Erro')
    ...
    st.markdown("<h3 style='text-align: center;'>Etapa de avaliacão</h3>", 
                unsafe_allow_html=True)
    
    st.image("./src/img/modelavaliation.png", caption="código para avaliação",
             use_container_width=True)
    ...
    st.html('''
            ...
            Brent utilizando aprendizado de máquina.
            Tem caráter acadêmico,e não é uma recomendação de compra / venda
            de ativos em bolsa de valores, assim como o resultado esperado 
            pode não contemplar variáveis importantes para melhores tomadas 
            de decisão.
            </i> <p>
            ''')
